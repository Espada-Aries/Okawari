...
select * from okwr_order_customer;
select * from okwr_order_customer where order_branch_id='fukuoka';
select order_menu_num, sum(order_count) from okwr_order_customer where order_branch_id='fukuokac' group by order_menu_num;
select rank(select sum(order_count) from okwr_order_customer where order_branch_id='fukuokac') from okwr_order_customer;

-- 메뉴 순위 또는 신품 등록
drop table okwr_rank cascade constraint;
create table okwr_rank(
	menu_num number		-- 메뉴번호
);
insert into okwr_rank values(1);
insert into okwr_rank values(2);
delete from okwr_rank;
select * from okwr_rank;
