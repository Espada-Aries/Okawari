...
public class CustomerMenuOrderAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 주문 내역에 아무것도 들어 있지 않은 경우
		if(cartList == null) {
			response.setContentType("text/html; charset=utf-8");
			PrintWriter out = response.getWriter();
			cartSession.removeAttribute("cartList");
			out.print("<script>");
			out.print("alert('注文履歴がないです。');location.href='customerPage.okawari';");
			out.print("</script>");
			out.flush();
		...
		List<StockBranchDTO> stockBranchList = customerOrderInsert.StockBranchList(stock_branch_id);
		// 주문 된것 원가 기입
		for (int i = 0; i < customerOrderList.size(); i++) {
			
			for (int j = 0; j < stockBranchList.size(); j++) {			
				if (customerOrderList.get(i).getOrder_menu_num() == stockBranchList.get(j).getStock_menu_num())
				customerOrderList.get(i).setOrder_oriprice(stockBranchList.get(j).getStock_cost());				
		...
		if (stockList.size() > 0) {
			...
			System.err.println("여긴 재고 검사하는 곳");
			response.setContentType("text/html; charset=utf-8");
			PrintWriter out = response.getWriter();
			out.print("<script>");
			...
			if (decCheck) {// DB 작업 (고객 주문내역 추가, 추가되었으면 고객번호 불러오기)
				...
				if (orderNumber != -1) {
					...
					out.print("alert('注文番号 " + orderNumber
							+ " でご注文できました。 ご利用ありがとうございます。');location.href='customerPage.okawari';");
					out.print("</script>");
					out.flush();
					...
					out.print("alert('申し訳ございません。只今、" + orderNumber
							+ "商品は用意されておりません。');location.href='customerPage.okawari';");
					out.print("</script>");
					out.flush();
				}
			}
