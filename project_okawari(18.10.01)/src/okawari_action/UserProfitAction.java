   ...
   public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
      ...
      if (request.getParameter("day") != null && request.getParameter("day") != "") {
         ...
         if (salesDTO != null) {
           ...
           open = 1;
            ...
            out.print("<script>");
            out.print("alert('ご選択の" + day
                  + " の日付に関する情報がございません。');location.href='userProfit.okawari';");
            out.print("</script>");
            out.flush();
      ...
      if (request.getParameter("dayUpdate") != null) {
         ...
         if (salesDTO != null) {
            ...
            if (salesList.size() > 0) {
               ...
               // rank update
               MenuRankAction mra = new MenuRankAction();
               mra.execute(request, response);
               
               PrintWriter out = response.getWriter();
               out.print("<script>");
               out.print(
                     "alert('売り上げをアップデートします。');location.href='userProfit.okawari';");
               out.print("</script>");
               out.flush();
            } else {
               // 해당자료가 없으면 신규 값 넣기
               ...
               out.print("alert('" + dayUpdate
                     + " 売り上げがアップデートできましあた。');location.href='userProfit.okawari';");
               out.print("</script>");
               out.flush();
           ...
           open = 1;
            ...
            PrintWriter out = response.getWriter();
            out.print("<script>");
            out.print("alert('ご選択の " + day + " の情報がございません。');location.href='userProfit.okawari';");
            out.print("</script>");
            out.flush();
            // 그날 매출 있는지 확인하기
         }
      ...
      if (request.getParameter("start") != null && request.getParameter("end") != null) {
    	  String start1 = request.getParameter("start");
  	   	...
  	   	System.out.println(start1.length() + " "  + end1.length());
         ...
         if (salesList.size() > 0) {
            ...
            PrintWriter out = response.getWriter();
            out.print("<script>");
            out.print("alert('ご選択の" + start + "から" + end
                  + " 間の情報がございません。');location.href='userProfit.okawari';");
            out.print("</script>");
            out.flush();
         }
      }
