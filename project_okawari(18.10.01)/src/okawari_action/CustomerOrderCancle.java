	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("day") != null && request.getParameter("orderNumber") != null
			...
			if (customerOrderList.size() > 0) {
				...
				return new ActionForward("customerOrderCancle.jsp", false);

			} else {
				response.setContentType("text/html; charset=utf-8");
				PrintWriter out = response.getWriter();
				out.print("<script>");
				out.print("alert('注文番号を確認して下さい。');location.href='customerOrderCancle.okawari';");
				out.print("</script>");
				out.flush();
			}
		} 
		...
		if (request.getParameter("cancle") != null) {
			...
			if (check) {
				...
				out.print("<script>");
				out.print(
						"alert('注文番号 " + orderNumber + " はキャンセルできました。');location.href='customerOrderCancle.okawari';");
				out.print("</script>");
				out.flush();
				...
				request.removeAttribute("orderNumber");
				out.print("<script>");
				out.print("alert('申し訳ございません。注文番号を確認して下さい。');location.href='customerOrderCancle.okawari';");
				out.print("</script>");
				out.flush();
			}
		...
		if (request.getParameter("day") == null && request.getParameter("orderNumber") == null
				&& request.getParameter("cancle") == null) {
			return new ActionForward("customerOrderCancle.jsp", false);
		}
