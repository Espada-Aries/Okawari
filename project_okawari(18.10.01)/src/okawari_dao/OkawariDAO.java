...
public class OkawariDAO {
	...
	public ArrayList<StockBranchDTO> StockBranchList(String stock_branch_id) {
		...
		StockBranchDTO dto = null;
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, stock_branch_id);
			rs = pstmt.executeQuery();
			
			while (rs.next()) {
				dto = new StockBranchDTO();
	...
	public ArrayList<UserDTO> showAllUser() {
		String sql = "select user_id from okwr_user where user_auth=1 or user_auth=2";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public int insertOrderCustomer(List<OrderDTO> customerOrderList) { // mr.oh
		...
		try {
			...
			for (int i = 0; i < customerOrderList.size(); i++) {
				...
				System.err.println(customerOrderList.get(i).getOrder_oriprice());
	...
	public ArrayList<OrderDTO> findCustomerOrder(String user_id, String day, int orderNumber) {
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.println(dto.getOrder_branch_id());
				System.out.println(dto.getOrder_menu_name());
				orderList.add(dto);
	...
	public ArrayList<OrderDTO> getCustomerOrderList(String user_id) {
		String sql = "select * from okwr_order_customer where order_branch_id=? and order_date=? order by order_time";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ArrayList<OrderDTO> list = new ArrayList<OrderDTO>();
		OrderDTO dto = null;
		GetDateTime today = new GetDateTime();
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, user_id);
			pstmt.setString(2, today.getDate());
			rs = pstmt.executeQuery();
			while(rs.next()) {
				dto = new OrderDTO();
				dto.setOrder_num(rs.getInt("order_num"));
	...
	public ArrayList<MenuRankDTO> selectPopularMenu() {
		String sql = "select order_menu_num, sum(order_count) from okwr_order_customer group by order_menu_num";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ArrayList<MenuRankDTO> list = new ArrayList<MenuRankDTO>();
		MenuRankDTO dto = null;
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()) {
				dto = new MenuRankDTO();
				dto.setMenu_num(rs.getInt("order_menu_num"));
				list.add(dto);
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	public void menuRankInsert(int menu_num) {
		String sql = "insert into okwr_rank values(?)";
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public void menuRankDelete() {
		String sql = "delete from okwr_rank";
		Connection conn = null;
		PreparedStatement pstmt = null;
		int deleteResult = 0;
		try {
	...
	public ArrayList<MenuRankDTO> menuRankSelect() {
		String sql = "select * from okwr_rank";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ArrayList<MenuRankDTO> list = new ArrayList<MenuRankDTO>();
	...
	public String findUserId(String user_name, String user_manager, String user_businum) {
		String sql = "select user_id from okwr_user where user_name=? and user_manager=? and user_businum=?";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			pstmt.setString(2, user_manager);
			pstmt.setString(3, user_businum);
			rs = pstmt.executeQuery();
			if(rs.next()) {
	...
	public String findUserPasswd(String user_id, String user_manager, String user_businum) {
		String sql = "select user_pass from okwr_user where user_id=? and user_manager=? and user_businum=?";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
