...
public class CustomerOrderService {
	...
	public ArrayList<MenuDTO> stockCheck(String stock_branch_id, List<OrderDTO> customerOrderList) {
		...
		String branch_id = "";
		for (int i = 0; i < stock_branch_id.length() - 1; i++) {
			char ch = stock_branch_id.charAt(i);
			branch_id += ch;
		}
		
		stockCheckList = dao.StockBranchList(branch_id); // 잘 작동
	...
	public ArrayList<StockBranchDTO> StockBranchList(String stock_branch_id) {
		ArrayList<StockBranchDTO> stockBranchList = new ArrayList<StockBranchDTO>();
		String branch_id = "";
		for (int i = 0; i < stock_branch_id.length() - 1; i++) {
			char ch = stock_branch_id.charAt(i);
			branch_id += ch;
		}
		stockBranchList = dao.StockBranchList(branch_id); // 잘 작동
		return stockBranchList;
	}

	public ArrayList<OrderDTO> getOrderList(String user_id) {
		OkawariDAO dao = new OkawariDAO();
		ArrayList<OrderDTO> list = dao.getCustomerOrderList(user_id);
		return list;
	}
